...
class LLMPolicyAdvisor:
    ...
    def get_policy(self, trajectories, returns, coverage_info, current_epsilon, avg_eval_reward, seed_val, past_policies):
        ...
        Environment: Gymnasium CliffWalking-v1 (is_slippery=True)
        ...
        Optimal return (shortest safe path) = -25
        ...
        - If trajectory return is very low (e.g., -100 or worse), agent likely fell into cliff
        - State 36 is START (near cliff)
        - Movements toward the bottom row are risky

        Task:
        - Identify where the agent is failing
        - Focus especially on states near cliff (35–46)
        - Improve only critical states
        - Avoid repeating bad policies
        - If similar states were already suggested in past policies and did not improve returns, choose different states
        
        Additional Guidance:
        - The goal is located at the far right of the grid
        - The agent should prioritize reaching the goal in minimum steps
        - Avoid loops or oscillations between states
        - Prefer moving RIGHT unless it leads directly into a cliff
        - Use UP only to escape danger, not as a default action
        - Do NOT assign the same action (e.g., LEFT or UP) for all states
        Output JSON only:
        {{
...
class Trainer:
    def __init__(self, hp, seed):
        ...
        env = gym.make("CliffWalking-v1", is_slippery=True, max_episode_steps=200)
        env.reset(seed=seed)
        ...
        def make_env():
            def _make():
                e = gym.make("CliffWalking-v1", is_slippery=True, max_episode_steps=200)
...
if __name__ == "__main__":
    ...
    HP = {
        ...
        "clip_grad_norm": 4.0,
        "eval_episodes": 20,
        "early_stop_threshold": -70.0,  # CliffWalking scale
